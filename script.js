...
// Sphere projected to disk
const sphere=document.getElementById('sphereCanvas'), sctx=sphere.getContext('2d'); let mouse={x:.5,y:.5};
sphere.addEventListener('pointermove',e=>{const r=sphere.getBoundingClientRect();mouse={x:(e.clientX-r.left)/r.width,y:(e.clientY-r.top)/r.height}});
function drawDiskField(ctx,w,h,tilt=0,eqY=null){
  ...
  for(let iy=-N;iy<=N;iy++)for(let ix=-N;ix<=N;ix++){
    ...
    arrow(ctx,x,y,z*18*scale,0,'#6de5ff',.48,.8);
    arrow(ctx,x,y,tilt*z*15*scale,-z*18*scale,'#ff6aa9',tilt===0?.25:.62,.8);
  }
...
function drawSphere(){const {w,h}=fitCanvas(sphere);sctx.clearRect(0,0,w,h);drawDiskField(sctx,w,h,.0,null);const cx=w/2,cy=h/2,R=Math.min(w,h)*.38;const X=(mouse.x-.5)*2*.85,Y=-(mouse.y-.5)*2*.85;if(X*X+Y*Y<.85){sctx.strokeStyle='#fff8';sctx.beginPath();sctx.arc(cx+X*R,cy-Y*R,12*DPR,0,Math.PI*2);sctx.stroke();}requestAnimationFrame(drawSphere)}drawSphere();
